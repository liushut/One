module ZJ {
    ...
    export class ResManager {
        ...
        public async init(stage: egret.Stage) {
            ...
            await this.loadConfig("resource/default.res.json?v=" + Math.random(), "resource/");
         ...
         */
        public loadDragonBones(fileName: string, armatureName: string = "", cb: (db: dragonBones.EgretArmatureDisplay) => void = null): dragonBones.EgretArmatureDisplay {
            if (armatureName == "") {
            ...
            if (this.dbFileNames.indexOf(fileName) == -1) {
                if (cb) {
                    this.loadRes(fileName + "_ske_json", (res, source) => {
                        let skeletonData = res;
                        this.loadRes(fileName + "_tex_json", (res, source) => {
                            let textureData = res;
                            this.loadRes(fileName + "_tex_png", (res, source) => {
                                let texture = res;
