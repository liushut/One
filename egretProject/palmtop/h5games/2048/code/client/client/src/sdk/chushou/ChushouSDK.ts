class ChushouSDK {
	...
	* "hostOpenUid" : "sl10702"
	...
	public csRecordReport(gameResult: string, cb?: (data: any) => void): void {
		...
		csRecordReport(`{"accessToken" : "${this.csAuthRequestData.accessToken}","openUid" : "${this.csAuthRequestData.openUid}","gameResult" : "${gameResult}" , "gameRecordId" : "${chushouGameConfig.gameRecordId}"}`, (data: string) => {
			this.log("cb csRecordReport")
			if (cb) { cb(JSON.parse(data)) }
		})
	}
