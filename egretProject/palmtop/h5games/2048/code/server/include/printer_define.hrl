...
-define(BADCSV(CSV, KEY), ?WARNING_MSG("BAD CSV:~p, Key:~p", [CSV, KEY]), {error, {badcsv, CSV, KEY}}).
-define(BADCSV2(CSV, KEY), ?WARNING_MSG("BAD CSV:~p, Key:~p", [CSV, KEY])).
-define(DEBUG_FILE(__Str), ?DEBUG_FILE(__Str, [])).
-define(DEBUG_FILE(__Format, __Args),
            file:write_file("../logs/debug.txt", lists:flatten(io_lib:format(__Format, __Args)), [append])
        ).
-endif.
