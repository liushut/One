...
write(2, [BattleId, MsgId, IsSuccess, GroupResultList, ScoreList]) ->
    Content = [
        {battleId, BattleId},
        {msgId, MsgId},
        {isSuccess, IsSuccess},
        {groupResult, {struct_array, GroupResultList}},
        {scoreResult, {struct_array, ScoreList}}
    ],
...
write(10002, [BattleId, MsgId, GroupResultList, ScoreList]) ->
    Content = [
        {battleId, BattleId},
        {msgId, MsgId},
        {groupResult, {struct_array, GroupResultList}},
...
write(10005, [BattleId, RoomId, Player, ResultStr, Duration, Level, Score2]) ->
    AppId = config:get_game_code(),
    Event = lists:concat(['game.',AppId]),
    Body = util:maps_to_json(#{
        appId => AppId,
        event => Event,
        params => #{
            accountname => Player#ali_battle_player.nickname,
            accountId => Player#ali_battle_player.open_id,
            war_id => tool:to_list(RoomId),
            war_result => ResultStr,
            speed_level => Level,
            'multi-row_eliminate_times' => Score2,
            duration => Duration
        }
    }),
    ?PRINT_MSG("10005 ~p",[Body]),
    pack_write(10005, BattleId, Body);
...
read_player([{struct, PlayerInfo} | Tail], List) ->
    ...
    RobotLevel =
        case proplists:get_value(<<"robotLevel">>, PlayerInfo) of
