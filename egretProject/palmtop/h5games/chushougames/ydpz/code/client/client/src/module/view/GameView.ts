class GameView extends ZJ.ViewBase {
	...
	private tailArray: eui.Group[] = [];//图片承载group数组。
	private tailImgArray: eui.Image[] = [];//图片数组
	...
	private showTopAndButtom() {
		...
		for (let i = 0; i <= num / 2 + 1; i++) {
			if (i == 0) {
				...
				let rect: egret.Rectangle = new egret.Rectangle(22, 116, 136, 7);
				img.scale9Grid = rect;
				img.width = 154;
				img.height = 450;
				...
				img.y = 123;
				this.topFather.addChild(img);
				let img1: eui.Image = new eui.Image("danjuchi_png");
				let rect1: egret.Rectangle = new egret.Rectangle(22, 116, 136, 7);
				img1.width = 154;
				img1.height = 450;
		...
		for (let i = 0; i <= num / 2 + 1; i++) {
			if (i == 0) {
				let img = new eui.Image("danjuchi_png");
				...
				this.backFather.addChild(img);
				let img1 = new eui.Image("danjuchi_png");
				let rect1: egret.Rectangle = new egret.Rectangle(22, 116, 136, 7);
				img1.scale9Grid = rect1;
				img1.width = 154;
				img1.height = 450;
	...
	private tailShow(jugde: number) {
		if (jugde == 0) {
			...
			//图片下的group做移动
			for (let i = 0, len = this.tailArray.length; i < len; i++) {
				let tail = this.tailArray[i]
				egret.Tween.get(tail).wait(16 * i).call(() => {//in  慢->快   out 快->慢   inout  man kuai man  时间越小，脱层  越大，等待时间长。
					let tw = egret.Tween.get(tail);
					tw.to({ y: 190 }, 70).
				...
				//图片数组 做缩放
				let tailImg = this.tailImgArray[i]
				egret.Tween.get(tailImg).wait(16 * i).call(() => {//in  慢->快   out 快->慢   inout  man kuai man  时间越小，脱层  越大，等待时间长。
					let tw = egret.Tween.get(tailImg);
					tw.to({ scaleX: 0.7, scaleY: 0.7 }, 70, egret.Ease.sineIn).
						to({ scaleX: 0, scaleY: 0 }, 100, (x: number) => {
			...
			for (let i = 0; i < this.tailArray.length; i++) {
				...
				this.tailImgArray[i].scaleX = 0.95;
				this.tailImgArray[i].scaleY = 0.95;
			}
	...
	private createQiuS() {
		if (this.isFirst) {
			...
			let imgQius = new eui.Image("xiaoqiu2_png");//小球图片
			...
			let maskImg = new eui.Image("daqiu_png");//大球图片
