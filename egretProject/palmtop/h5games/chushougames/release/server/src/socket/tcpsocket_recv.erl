...
init(Socket) ->
    ...
    {ok, HandleState} = client_handler:init([self(), tcpsocket, Ip, Port]),
    State = #tcp_state{
        tcp_socket = Socket,
...
terminate(Reason, State) ->
    client_handler:terminate(Reason, State#tcp_state.handle_state),
    ok.
...
do_info({inet_async, Socket, _Ref, {ok, Binary}}, State) ->
    ...
    {ok, NewHandleState} = client_handler:router_package(Binary, State#tcp_state.handle_state),
...
do_info({'EXIT', _Port, _Reason}, State) ->
    {stop, normal, State};

do_info({socket_close}, State) ->
    {stop, normal, State};

do_info(Info, State) ->
    case client_handler:info(Info, State#tcp_state.handle_state) of
        {ok, NewState} ->
            ok;
        NewState when is_record(NewState, ets_socket_state) ->
            ok;
        _ ->
            NewState = State#tcp_state.handle_state
    end,
    {noreply, State#tcp_state{handle_state = NewState}}.
