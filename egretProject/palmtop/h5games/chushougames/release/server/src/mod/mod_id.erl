...
get_player_id(OpenId) ->
    case ets:lookup(?ETS_PLAYER_ID, OpenId) of
        [{OpenId, PlayerId} | _] ->
            PlayerId;
        _ ->
            PtId = config:get_pt_id(),
            case db_player:get_id_by_open_id(OpenId, PtId) of
                PlayerId when is_integer(PlayerId) ->
                    ets:insert(?ETS_PLAYER_ID, {OpenId, PlayerId}),
                    PlayerId;
                _ ->
                    0
            end
...
init([]) ->
    ...
    ets:new(?ETS_PLAYER_ID, [{keypos, 1}, named_table, public, set]),
