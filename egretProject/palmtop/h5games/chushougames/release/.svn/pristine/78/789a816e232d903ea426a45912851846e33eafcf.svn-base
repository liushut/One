...
set_today_start() ->
    Now = util:unixtime(),
    {Date, Time} = util:seconds_to_localtime(Now),
    Seconds = calendar:time_to_seconds(Time),
    TodayStart = Now - Seconds,
    DayOfWeek = calendar:day_of_the_week(Date),
    WeekStart = TodayStart - (DayOfWeek - 1) * 86400,
    ets:insert(ets_timer, {week_start, WeekStart}),
...
event_clock(State) ->
    ...
    ets:insert(ets_timer, {now_seconds, NowSeconds}),
    ets:insert(ets_timer, {timer, {NowSecs, Time_Since_Last_Call, NowUniversalTime, NowLocalTime}}),
    {{LYear, LMonth, LDay},{LHours, LMin, LSeconds}} = util:seconds_to_localtime(NowSeconds),
    ets:insert(ets_timer, {local_time, {{LYear, LMonth, LDay},{LHours, LMin, LSeconds}}}),
    ets:insert(ets_timer, {local_date, lists:concat([LYear,"-",LMonth,"-",LDay," ",LHours,":",LMin,":",LSeconds])}),
    State.