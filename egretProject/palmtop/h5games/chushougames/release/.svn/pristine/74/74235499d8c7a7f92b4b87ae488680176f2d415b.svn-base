...
init(Opts) ->
    ...
    ChartSet       = proplists:get_value(chartset, Opts, "utf8"),
    ProcessName    = proplists:get_value(process_name, Opts),
    ...
    case Result of
        {ok, Handshake, SockMod, Socket} ->
            ...
            MysqlState = #ets_mysql_state{
                pid = self(),
                server = Type,
                process_name = ProcessName,
                db_name = Database
            },
            handle_call({query, "set names " ++ ChartSet, State#state.query_timeout}, self(), State),
            ets:insert(?ETS_MYSQL_STATE, MysqlState),
            {ok, State1};
        #error{} = E ->
...
handle_info({'EXIT', _From, Reason}, State) ->
    stop_server({process_error, Reason}, State);
...
terminate(Reason, #state{socket = Socket, sockmod = SockMod})
    ...
    SockMod:setopts(Socket, [{active, once}]),
    ets:delete(?ETS_MYSQL_STATE, self()),
    R;
...
query_call(Conn, CallReq) ->
    query_call(Conn, CallReq, 60000).
query_call(Conn, CallReq, To) ->
    NewTo = if
        is_integer(To) ->
            To;
        true ->
    ...
    case gen_server:call(Conn, CallReq, NewTo) of
