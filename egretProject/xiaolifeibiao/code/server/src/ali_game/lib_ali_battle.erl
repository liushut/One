...
update_player_score_result_data(NewScore, ResultData, OpenId) ->
    PlayerId = mod_id:get_player_id(OpenId),
    Player = get_battle_player(PlayerId),
    NewPlayer = Player#ali_battle_player{
        score = NewScore,
...
battle_force_quit(BattleState, MsgId) ->
    PlayerList = get_battle_player(),
    GroupResultList = battle_end_result(PlayerList, 0, []),
    {ok, SendBin} = pt_ali_head:write(2, [BattleState#ets_battle_state.battle_id, MsgId, true, GroupResultList]),
    lib_send:send_to_sid(BattleState#ets_battle_state.socket_pid, SendBin),
...
battle_finish(BattleState, TmpWinPlayerId) ->
    ...
    PlayerList = get_battle_player(),
    GroupResultList = battle_end_result(PlayerList, WinPlayerId, []),
    MsgId = mod_id:get_msg_id(),
    erlang:send_after(20000, self(), {close_battle}),
    {ok, EndData} = pt_ali_head:write(10002, [BattleState#ets_battle_state.battle_id, MsgId, GroupResultList]),
    lib_send:send_to_sid(BattleState#ets_battle_state.socket_pid, EndData, 2000).

battle_end_result([], _, List) ->
    List;
battle_end_result([BattlePlayer|T], WinPlayerId, GroupResultList) ->
    Result =
        if
            WinPlayerId == 0 ->
                0;
    ...
    battle_end_result(T, WinPlayerId, [GroupResult | GroupResultList]).